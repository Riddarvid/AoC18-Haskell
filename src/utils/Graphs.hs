...
exploreUntil' :: (Ord a) => BFSOptions a -> (BFSState a -> Bool) -> BFSState a -> BFSState a
exploreUntil' options cond startState = endState
  where
    endState = until (\state -> cond state || Set.null (lastLayer state)) (stepBFS options) startState
...
reachableBFSLimit :: (Ord a) => a -> Int -> AdjacencyFun a -> BFSOptions a -> Set a
reachableBFSLimit start limit adjacency options = Map.keysSet $ visited endState
  where
    startState = startStateSingle start adjacency
    endCond state' = Map.size (visited state') >= limit
    endState = exploreUntil' options endCond startState
